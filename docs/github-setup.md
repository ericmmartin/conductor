...
2. **A Status single-select field** with these options at minimum: `Ready`, `In Progress`, `In Review`, `Validating`, `Blocked`, `Done`.
3. **A `Ready At` Date field** (custom) that conductor writes on first observation of each issue at status `Ready`. This is the FIFO sort key — see below.
